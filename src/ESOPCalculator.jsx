            ...
            <div className="space-y-6">
                ...
                <div className="bg-white/80 backdrop-blur-xl rounded-2xl shadow-lg p-6 border border-gray-100">
                  <div className="flex items-center justify-between mb-5">
                    <h3 className="text-lg font-bold text-gray-800 flex items-center gap-2">
                      ...
                      {filingStatus === 'MFJ' ? 'Your Paycheck' : `Your Paycheck (${payFreq})`}
                    </h3>
                    <span className="text-xs font-semibold px-2 py-1 bg-green-100 text-green-700 rounded-full">$ USD</span>
                  </div>
                  {filingStatus === 'MFJ' && (
              ...
              {activeTab === 'income-tax' && (
                <div className="bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-600 rounded-2xl shadow-2xl p-8 text-white">
                  ...
                  {/* 401(k) Total Contributions */}
                  <div className="bg-white/10 rounded-xl p-4">
                    <h4 className="text-sm text-indigo-100 mb-4 font-semibold uppercase tracking-wide">401(k) Total Annual Contributions</h4>
                    <div className={`grid gap-6 ${filingStatus === 'MFJ' ? 'grid-cols-3' : 'grid-cols-1'}`}>
                      <div>
                        <p className="text-sm text-indigo-200 mb-1">{filingStatus === 'MFJ' ? 'Your Total' : 'Total 401(k)'}</p>
                        <p className="text-xl font-bold">{fmtUSD0((k401Employee || 0) + (usTax.matchAnnual || 0))}</p>
                        <p className="text-xs text-indigo-300 mt-1">
                          Employee: {fmtUSD0(k401Employee || 0)} + Match: {fmtUSD0(usTax.matchAnnual || 0)}
                        </p>
                      </div>
                      {filingStatus === 'MFJ' && (
                        <>
                          <div>
                            <p className="text-sm text-indigo-200 mb-1">Spouse's Total</p>
                            <p className="text-xl font-bold">{fmtUSD0((spouseK401Employee || 0) + (usTax.spouseMatchAnnual || 0))}</p>
                            <p className="text-xs text-indigo-300 mt-1">
                              Employee: {fmtUSD0(spouseK401Employee || 0)} + Match: {fmtUSD0(usTax.spouseMatchAnnual || 0)}
                            </p>
                          </div>
                          <div className="text-right">
                            <p className="text-sm text-indigo-200 mb-1">Combined Total</p>
                            <p className="text-xl font-bold">{fmtUSD0((k401Employee || 0) + (usTax.matchAnnual || 0) + (spouseK401Employee || 0) + (usTax.spouseMatchAnnual || 0))}</p>
                            <p className="text-xs text-indigo-300 mt-1">
                              Employee: {fmtUSD0((k401Employee || 0) + (spouseK401Employee || 0))} + Match: {fmtUSD0((usTax.matchAnnual || 0) + (usTax.spouseMatchAnnual || 0))}
                            </p>
                          </div>
                        </>
                      )}
                  ...
                  <p className="text-xs text-indigo-200 mt-4 bg-white/10 p-3 rounded-lg">
                    <strong>Note:</strong> 401(k) contributions (both employee and employer match) are not taxed when contributed but will be taxed upon withdrawal in retirement.
                  </p>
                </div>
              )}
            </div>
